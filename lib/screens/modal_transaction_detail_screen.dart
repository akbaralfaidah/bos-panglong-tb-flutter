...
class ModalTransactionDetailScreen extends StatefulWidget {
  ...
  const ModalTransactionDetailScreen({
    super.key,
  ...
  @override
  State<ModalTransactionDetailScreen> createState() =>
      _ModalTransactionDetailScreenState();
}

class _ModalTransactionDetailScreenState extends State<ModalTransactionDetailScreen>
    ...
    if (mounted) {
      setState(() {
        ...
        _items = rawItems.map<Map<String, dynamic>>((e) {
          ...
          // 🔥 OVERRIDE WITH MODAL PRICES 🔥
          double capPrice = (item['capital_price'] as num?)?.toDouble() ?? 0;
          double displayQty = (item['request_qty'] as num?)?.toDouble() ?? (item['quantity'] as num?)?.toDouble() ?? 0;
          
          item['sell_price'] = capPrice.round();
          item['agreed_total'] = (capPrice * displayQty).round();

          return item;
        }).toList();

        // RECALCULATE TOTAL MODAL
        int calcTotal = _opCost;
        for (var item in _items) {
           calcTotal += (item['agreed_total'] as num?)?.toInt() ?? 0;
        }
        _totalPrice = calcTotal;
        _discount = 0; // Disable discount for modal
